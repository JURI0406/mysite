...
def create_trade_book(_df,_codes,_sig_dict) : 
    ...
    for date, codes in _sig_dict.items() :
        ...
        # codes 반복문 생성
        for code in codes :
            # book에서 index가 date인 컬럼이 r_code인 컬럼에 준비 내역을 추가
...
def multi_return(_df,_codes):
    ...
    for idx in df.index : 
        # 종목별로 매수/매도 확인
        for code in _codes :
            # 매수의 조건 : 2행 전의 p_code가 ""
            #            1행 전의 p_code가 'ready_code'
